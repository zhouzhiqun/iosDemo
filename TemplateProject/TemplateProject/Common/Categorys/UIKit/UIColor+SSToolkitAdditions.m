...
+ (UIColor *)ss_colorWithHex:(NSString *)hex {
    ...
    CGFloat red     = [[hex substringWithRange:NSMakeRange(0, 2)] ss_hexValue] / 255.0f;
    CGFloat green   = [[hex substringWithRange:NSMakeRange(2, 2)] ss_hexValue] / 255.0f;
    CGFloat blue    = [[hex substringWithRange:NSMakeRange(4, 2)] ss_hexValue] / 255.0f;
    CGFloat alpha   = [[hex substringWithRange:NSMakeRange(6, 2)] ss_hexValue] / 255.0f;
...
- (NSString *)ss_hexValue {
    return [self ss_hexValueWithAlpha:NO];
}
...
- (NSString *)ss_hexValueWithAlpha:(BOOL)includeAlpha {
    ...
    // Add alpha
    if (hex && includeAlpha) {
        hex = [hex stringByAppendingFormat:@"%02x", (int)(self.alpha * 255.0f)];
    }
